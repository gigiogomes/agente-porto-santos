...
def _normalize_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if set(colunas_base).issubset(df.columns):
        ...
        # 3. Mescla e salva na coluna "ESTADO"
...
def load_port_data():
    ...
    if not candidates:
        error_details = "\n".join(
            [f"- {d['source']}: {d.get('error', 'Erro desconhecido')}" for d in diagnostics if d["status"] == "error"]
        )
...
# =================================================================
# CAGR DETERMINÍSTICO (calculado no pandas, nunca pelo LLM)
# =================================================================
def _detect_complete_years(df_scope: pd.DataFrame) -> set:
    """
    Um ano é 'completo' quando a base tem os 12 meses presentes (disponibilidade
    port-wide). Avaliado sobre o conjunto inteiro, não por terminal, porque
    completude é propriedade da fonte de dados, não da atividade do terminal.
    """
    if df_scope.empty:
        return set()
    months = df_scope.groupby("ANO")["MES"].nunique()
    return {int(ano) for ano, n in months.items() if n >= 12}
...
def _compute_cagr(
    ...
    """
    CAGR = (valor_final / valor_inicial) ** (1 / anos) - 1, usando os endpoints
    de ANOS COMPLETOS dentro da janela. Retorna tabela transparente com os anos
    e volumes efetivamente usados, para o bot repassar sem inventar.
    """
    val_col = "TOTAL_TEU"

    # Completude avaliada port-wide na janela pedida.
    scope = df_full
    if start_date:
        scope = scope[scope["DATE"] >= pd.to_datetime(start_date)]
    if end_date:
        scope = scope[scope["DATE"] <= pd.to_datetime(end_date)]

    complete_years = _detect_complete_years(scope)
    if len(complete_years) < 2:
        return (
            "[AVISO DO SISTEMA] Não há anos completos suficientes (>= 2) para calcular o CAGR "
            f"na janela informada. Anos completos detectados: {sorted(complete_years) or 'nenhum'}. "
            "O bot deve informar isso ao usuário e NÃO deve estimar o valor por conta própria."
        )
    ...
    dfc = df_filtered[df_filtered["ANO"].isin(complete_years)]

    gb = [group_by] if isinstance(group_by, str) else (group_by or [])
    by_terminal = "terminal" in gb

    if by_terminal:
        groups = [(sigla, g) for sigla, g in dfc.groupby("TERMINAL_SIGLA_CANONICA")]
    else:
        groups = [(None, dfc)]

    rows = []
    for label, g in groups:
        yearly = g.groupby("ANO")[val_col].sum()
        v0 = yearly.get(first_year)
        vn = yearly.get(last_year)

        row = {}
        if by_terminal:
            row["Terminal"] = label if label is not None else "N/D"
        row["Ano Inicial"] = first_year
        row["TEU Inicial"] = int(round(v0)) if v0 is not None else None
        row["Ano Final"] = last_year
        row["TEU Final"] = int(round(vn)) if vn is not None else None
        row["Anos"] = n

        if v0 is None or vn is None or v0 <= 0:
            row["CAGR (%)"] = "N/D (sem movimentação no ano inicial)"
        else:
            cagr = (vn / v0) ** (1.0 / n) - 1.0
            row["CAGR (%)"] = f"{round(cagr * 100, 2)}%"
        rows.append(row)
...
def query_port_data(
    ...
    if df_filtered.empty:
        return (
            f"[AVISO DO SISTEMA] Nenhum dado encontrado. Filtros usados: Terminais={terminals}, "
            f"CargoMix={cargo_mix_filter}, Periodo={start_date} a {end_date}. O bot deve informar isso ao usuário."
        )

    # 2.5 CAGR é determinístico: calcula no pandas e retorna (não cai no fluxo de soma).
    if metric == "cagr":
        return _compute_cagr(df, df_filtered, group_by, start_date, end_date)
